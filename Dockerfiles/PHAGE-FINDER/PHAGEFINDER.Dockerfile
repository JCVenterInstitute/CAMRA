...
RUN apt update && \
    apt upgrade -y && \
    apt install -y wget libssl-dev perl cpanminus build-essential libxml2 linux-headers-generic blender libgraphics-colorobject-perl libgomp1 python3 python3-pip infernal-doc autoconf git build-essential musl dietlibc-dev && \
