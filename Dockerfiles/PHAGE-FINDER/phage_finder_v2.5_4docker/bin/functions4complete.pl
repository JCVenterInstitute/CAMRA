...
sub draw_gene_pairs {
    my($a1, $a2, $name1, $name2, $percent_id, $type, $color_code) = @_; 
    my $inc = 0;  
    my($x1, $x2, $x3, $y1, $y2, $depth);
    my $max_colors = $total_colors; #default
    return unless (exists $gene{$a1}{$name1}{'x1'} && exists $gene{$a2}{$name2}{'x1'}); 
	...
	$color_code = &position_to_color(1-$ratio,$total_colors); # flip the ratio so that 100% match is 1 and 20% is 0
    }
    if ($type eq "pairs") {
	...
	printf("2 3 0 1 %d %d %d 0 20 0.000 0 0 0 0 0 5\n",	       
	      ...
	      $color_code, 
       	      $depth);
	if ($gene{$a2}{$name2}{'y2'} < $gene{$a1}{$name1}{'y2'}) {
            printf("\t %d %d %d %d %d %d %d %d %d %d\n",
	    $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y2'},
	    $gene{$a1}{$name1}{'x2'},$gene{$a1}{$name1}{'y2'},
	    $gene{$a2}{$name2}{'x2'},$gene{$a2}{$name2}{'y3'},
	    $gene{$a2}{$name2}{'x1'},$gene{$a2}{$name2}{'y3'},
	    $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y2'});
        }
        else {
            printf("\t %d %d %d %d %d %d %d %d %d %d\n",
	    $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y3'},
	    $gene{$a1}{$name1}{'x2'},$gene{$a1}{$name1}{'y3'},
	    $gene{$a2}{$name2}{'x2'},$gene{$a2}{$name2}{'y2'},
	    $gene{$a2}{$name2}{'x1'},$gene{$a2}{$name2}{'y2'},
	    $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y3'});
        }
    }
    else {
        ...
        printf("2 3 0 1 %d %d %d 0 20 0.000 0 0 0 0 0 9\n",	       
	      ...
	      $color_code, 
       	      $depth);
        if ($gene{$a2}{$name2}{'y2'} < $gene{$a1}{$name1}{'y2'}) {# changed, NOT tested dfouts
	    printf("\t %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
		   $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y1'},
		   $gene{$a1}{$name1}{'x2'},$gene{$a1}{$name1}{'y1'},
		   $gene{$a1}{$name1}{'x2'},$gene{$a1}{$name1}{'y2'},
		   $gene{$a2}{$name2}{'x2'},$gene{$a2}{$name2}{'y3'},
		   $gene{$a2}{$name2}{'x2'},$gene{$a2}{$name2}{'y1'},
		   $gene{$a2}{$name2}{'x1'},$gene{$a2}{$name2}{'y1'},
		   $gene{$a2}{$name2}{'x1'},$gene{$a2}{$name2}{'y3'},
		   $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y2'},
		   $gene{$a1}{$name1}{'x1'},$gene{$a1}{$name1}{'y1'});
	}
...
sub fake_genes { # determine the proper level structure and coords for regions (converts end5, end3 to fig units)
    ...
    $x1 = &x_transform($gene_ref->{'end5'}, $level_ref);
    $x2 = &x_transform($gene_ref->{'end3'}, $level_ref);
...
sub draw_level { 
    my ($level_ref,$fragmt_ref) = @_;
    my ($x1, $x2, $y);

    $x1 = &x_transform($fragmt_ref->{'lend'}, $level_ref);
    $x2 = &x_transform($fragmt_ref->{'rend'}, $level_ref);

	return $x2 if $global_struct->{'horizontal line'} == 0;

    if ($global_struct->{'tickmark'} == 0) {
	$y = &line2coord($fragmt_ref->{'level'}) + ($global_struct->{'gene_arrow_height'} / 2);
    }
    else {
	$y = &line2coord($fragmt_ref->{'level'});
    }
...
sub draw_ticks { # dfouts
  ...
  $y1 = &line2coord($level_ref->{'level'})+7;
  ...
  for ($position; $position <= $frag_end; $position = $position + $tick)  {
      ...
      $x1 = &x_transform($position, $level_ref);
...
sub read_genes {
  ...
  while (<IN>) {
    ...
    $gene{$frag_id}{$feat_name}{'end5'} = $end5;
    $gene{$frag_id}{$feat_name}{'end3'} = $end3;
    $gene{$frag_id}{$feat_name}{'name'} = $feat_name;
    $gene{$frag_id}{$feat_name}{'main_role'} = $role;
    $gene{$frag_id}{$feat_name}{'type'} = $type;
    $gene{$frag_id}{$feat_name}{'final_name'} = &choose_name($type, $feat_name, $gene_sym, $locus);
    #print STDERR "$end5, $end3, $feat_name, $gene_sym, $locus!, $type, $role, $frag_id, $gene{$frag_id}{$feat_name}{'final_name'}\n";
  }    
  close(IN);
}

sub read_chi_square {
	return unless $global_struct->{'chi_file'};
	...
	for $fragmt_id (keys %chi) {
		my @sort = sort {$a<=>$b} values %{$chi{$fragmt_id}} ;
		my $range = ($sort[$#sort] - $sort[0]) + 1;
                print STDERR "Range = <$range>\n";

		while (my($key, $value) = each %{$chi{$fragmt_id}}) {
		    $chi{$fragmt_id}{$key} = ($value-$sort[0])/$range;
		}
	} 
}
...
sub draw_gene_elements {
    ...
    foreach my $element (keys %{$gene{$fragmt_id}}) {				
    	$end5 = $gene{$fragmt_id}{$element}{end5};
		$end3 = $gene{$fragmt_id}{$element}{end3};
    	if (overlap($fragmt_ref->{lend},$fragmt_ref->{rend},$end5,$end3)) {	
	    	if (!$global_struct->{'select_genes_toggle'} or 
	    		($global_struct->{'select_genes_toggle'} and
	    			$global_struct->{'selected_genes'}->{$gene{$fragmt_id}{$element}{'name'}})) {
    
	    		$filled = 20;  # filled
	    		$filled = 41;  # pattern
	    		$filled = -1;  # no fill	    

	   		if (passes_length_test($end5, $end3)) {
		             $role = $gene{$fragmt_id}{$element}{'main_role'};
				 ...
				 if ((!defined($color)) && ($gene{$fragmt_id}{$element}{type} eq "ORF"))  {
	   		          #unless (($color) | ($gene{$fragmt_id}{$element}{type} eq "RBS") | ($gene{$fragmt_id}{$element}{type} eq "TERM") | ($gene{$fragmt_id}{$element}{type} =~ /RNA/) | ($gene{$fragmt_id}{$element}{type} eq "PMARK") | ($gene{$fragmt_id}{$element}{type} eq "PROM")){
				       printf STDERR ("can\'t find color for: %s :: %s drawing in black\n",
			       	       $role, $gene{$fragmt_id}{$element}{'name'});
			...
			if (($gene{$fragmt_id}{$element}{'type'} eq "ORF") && ($global_struct->{'arrowstyle'})){
	    		     &draw_genes($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
			}
			elsif ($gene{$fragmt_id}{$element}{'type'} eq "ORF"){
	    		     &draw_old_genes($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
			}
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "ORFNA") { # no arrowhead desired
	    		     &draw_gene_block($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
			   }
			elsif ($gene{$fragmt_id}{$element}{'type'} eq "CLS"){
			     $filled = 0; # force white fill color independent of role_id
	    		     &draw_clusters($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled,$role);
			}
			elsif ($gene{$fragmt_id}{$element}{'type'} =~ /RNA/) {
                             &draw_RNA($level_ref, $gene{$fragmt_id}{$element}{'type'}, $gene{$fragmt_id}{$element}{'end5'}, $gene{$fragmt_id}{$element}{'end3'}, $gene{$fragmt_id}{$element}{'name'}, $gene{$fragmt_id}{$element}{'final_name'});
			}elsif ($gene{$fragmt_id}{$element}{'type'} eq "PROM"){ # if promoter
                             $color = $global_struct->{'green'};
                             $pen = $global_struct->{'green'};
			     &draw_promoters($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "PRI"){ # if primer
                             $color = $global_struct->{'red'};
                             $pen = $global_struct->{'red'};
			     &draw_primers($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "PRI2"){ # if primer
                             $color = $global_struct->{'magenta'};
                             $pen = $global_struct->{'magenta'};
			     &draw_primers($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "TERM"){ # if terminator
                            $color = $global_struct->{'red'};
                            $pen = $global_struct->{'red'};
			    &draw_terminators($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "PMARK"){ # if contig break
                            $color = $global_struct->{'red'};
                            $pen = $global_struct->{'red'};
			    &draw_PMARKs($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "TSD")  { # if target site duplication
                            $color = $global_struct->{'black'};
                            $pen = $global_struct->{'black'};
                            &draw_TSDs($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "ENZ"){ # if restriction enzyme site
                            $color = $global_struct->{'blue'};
                            $pen = $global_struct->{'blue'};
			    &draw_enzymes($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
                        elsif ($gene{$fragmt_id}{$element}{'type'} eq "ATTP"){ # if restriction enzyme site
                            $color = $global_struct->{'cyan'};
                            $pen = $global_struct->{'cyan'};
			    &draw_ATTP($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
                        }
			elsif ($gene{$fragmt_id}{$element}{'type'} eq "REGION") {
			    &fake_genes($level_ref,$gene{$fragmt_id}{$element});
			}
			elsif ($gene{$fragmt_id}{$element}{'type'} eq "INDEL") {
			    $pen = $global_struct->{'black'};
			    $color = $global_struct->{'black'};
			    $filled = 20;
			    &draw_gene_block($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
			}
			elsif ($gene{$fragmt_id}{$element}{'type'} eq "Ti") {
			    $pen = $global_struct->{'red'};
			    $color = $global_struct->{'red'};
			    $filled = 20;
			    &draw_gene_block($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
			}
			elsif ($gene{$fragmt_id}{$element}{'type'} eq "Tv") {
			    $pen = $global_struct->{'blue'};
			    $color = $global_struct->{'blue'};
			    $filled = 20;
			    &draw_gene_block($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
...
sub draw_chi_square {
    ...
    my ($x, $y1, $y2, $y3);
    for my $position (keys %{$chi{$fragmt_id}}) {
    	if (&overlap($fragmt_ref->{lend},$fragmt_ref->{rend},$position,$position))  {
    		$x = &x_transform($position, $level_ref);
    		$y1 = &line2coord($level_ref->{'level'})+$global_struct->{'chi_position'};
		$y3 = $y1 + ($global_struct->{'gene_arrow_height'} / 2) + 300 + ($global_struct->{'chi_height'}/4); # needed for correct position of label
		#$y2 = $y3-$chi{$fragmt_id}{$position}*($global_struct->{'chi_height'}/2); # for upward-facing peaks
    		$y2 = $y3+$chi{$fragmt_id}{$position}*$global_struct->{'chi_height'}; # for downward-facing peaks
...
#################################
    ...
    $y1 = &line2coord($level_ref->{'level'}) + ($global_struct->{'gene_arrow_height'} / 2);;
    $y2 = $y1 - ($global_struct->{'gene_height'} / 2);
    $y3 = $y1 + ($global_struct->{'gene_height'} / 2);
...
#draw_genes($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
    ...
    $x1 = &x_transform($gene_ref->{'end5'}, $level_ref);
    $x3 = &x_transform($gene_ref->{'end3'}, $level_ref);
    $x2 = &calc_arrow_x($x1, $x3, $gene_ref->{'end5'}, $gene_ref->{'end3'}, $level_ref->{'orientation'});
...
#draw_genes($level_ref,$gene{$fragmt_id}{$element},$color,$pen,$filled);
    ...
    $y1 = &line2coord($level_ref->{'level'}); #Take -600 off for horizontal line above genes
...
sub draw_RNA {
    ...
    $width = $x2-$x1;
    $x = &x_transform($x1 + ($width/2), $level_ref);
    $y = &line2coord($level_ref->{'level'}) - ($global_struct->{'gene_height'}/6);

    $label_y = $y + ($height);
    if (!$global_struct->{'tickmark'})  { # if no tickmarks
	$y = &line2coord($level_ref->{'level'});# - ($global_struct->{'gene_height'}/2);
	$label_y = $y - ($height);
    }
    ...
    $label_y = $y - ($height+$height/4);
    if (!$global_struct->{'arrowstyle'})  { # if traditional arrowhead for genes
	$label_y = $y - ($height/4);
    }
    &add_label($final_name, $x, $x, $label_y, 1, 1, 0, 15);
...
sub draw_terminators { # dfouts
  ...
  $y1 = &line2coord($level_ref->{'level'})-14;
...
sub text {
    my($text_size,$justification,$angle,$x,$y,$text) = @_;
...
sub fig_header {
    ...
    if ($color_match eq "s")  {
	print STDERR "----------------------------------------------- ;)\n";
	my $spectrum = &init_color_spectrum(); # fill hash global %spectrum with rgb colors from 71 up
